...
| `VirtualVocationsConnector` | [VirtualVocations](https://www.virtualvocations.com/jobs/s-date) | Guest job sitemap; newest-first `lastmod`; first-stale stop; engineering title from slug; listing skip before detail; guest Job Summary; apply review-capped |
...
| `RemoteWlbConnector` | [RemoteWLB](https://remotewlb.com/jobs) | Guest monthly job sitemaps; reverse oldest-first `lastmod`; first-stale stop; engineering title filter; JobPosting JSON-LD; apply review-capped |
| `OmniJobsConnector` | [OmniJobs](https://omnijobs.io/en/search?location=US&locationType=remote&jobFunction=software+development) | Guest Playwright Chrome (Vercel checkpoint); remote + software development + US/LATAM/world/Europe; newest-first first-stale when `Opened` dates descend; stop at OmniJobs Pro; engineering title filter; listing skip before detail; apply review-capped |
