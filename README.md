...
| [JustJoin](https://justjoin.it/job-offers/remote?remote-work-options=hybrid&experience-levels=mid,senior,team-leader-manager&languages=en&sortBy=newest) | JSON offers API | Guest `GET /api/candidate-api/offers`. Mid/senior/lead, English, remote+hybrid (office dropped client-side). Newest-first `publishedAt` cursor; stop at first stale page. Job-page JSON-LD description. Employer `applyUrl` when present. |
...
| `rescore --promote` | Also move review jobs with fit_score ≥ 60 to shortlisted |
