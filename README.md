...
| [Ladders](https://www.theladders.com/jobs/searchresults-jobs?keywords=Software%20Engineer&sortBy=PUBLICATION_DATE&daysPublished=7&remoteFlags=Remote) | Playwright Chrome | Guest Newest remote search. Cloudflare blocks `requests`; installed Chrome is the default fetch. `/api/*` and `/job/*/apply` are robots-disallowed. Unique `target_roles` plus `software engineer`. Engineering title filter (keyword search leaks sales). Newest-first; stop at first stale job. Walk until an empty page or a repeated first card. A hung listing (15s) uses cards already in the HTML, otherwise retries on a new tab; two failed listings stop that query. Skip detail when listing location/title is enough. Apply is on Ladders (capped at review). |
...
| [VirtualVocations](https://www.virtualvocations.com/jobs/s-date) | Job sitemap | Guest `GET /sitemap/index/sitemap.xml` (no login). Newest-first `lastmod`; stop at first stale job. Engineering title from slug. Skip known URLs. Skip detail when listing title/`Remote` fails inclusion; guest Job Summary on detail (retried). Career-level path stays out of fetch. Apply/company membership-gated (capped at review). |
...
| [RemoteWLB](https://remotewlb.com/jobs) | Job sitemaps | Guest monthly `/sitemaps/jobs/YYYY-MM-1` (listing HTML is a JS shell). Month urlsets are oldest-first — reverse to newest-first; stop at first stale `lastmod`. Engineering title filter. JobPosting JSON-LD for description/company. `location` is `Remote`. `directApply` false (capped at review). |
| [OmniJobs](https://omnijobs.io/en/search?location=US&location=ANYWHERE_IN_LATAM&location=ANYWHERE_IN_WORLD&location=ANYWHERE_IN_EUROPE&locationType=remote&jobFunction=software+development) | Playwright Chrome | Guest remote software-development search. Vercel checkpoint blocks `requests`; installed Chrome is the default fetch. `Opened … ago` newest-first; stop at the first stale card when dates descend. Free preview ends at OmniJobs Pro (no further pages). Engineering title filter. Skip detail when listing title/location/JD is enough. Employer `applicationUrl` when present. Full results are Pro (capped at review). |
