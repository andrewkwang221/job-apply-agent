...
| [FlexJobs](https://www.flexjobs.com) | Next.js search HTML | Opt-in (`--source flexjobs`, not in `all`). Playwright login via `FLEXJOBS_EMAIL` / `FLEXJOBS_PASSWORD`; homepage search sorted by date; apply is subscription-gated |
...
| [Dice](https://www.dice.com/jobs?filters.workplaceTypes=Remote%7CHybrid) | Dice MCP | Guest `search_jobs` (no login). Profile `target_roles` + `keywords`, Remote+Hybrid, `sort=datePosted`. Newest-first; stop at first stale page. Full description from job-detail HTML. Apply is on Dice. |
...
| [Ladders](https://www.theladders.com/jobs/searchresults-jobs?keywords=Software%20Engineer&sortBy=PUBLICATION_DATE&daysPublished=7&remoteFlags=Remote) | Playwright Chrome | Guest Newest remote search. Cloudflare blocks `requests`; installed Chrome is the default fetch. `/api/*` and `/job/*/apply` are robots-disallowed. Unique `target_roles` plus `software engineer`. Engineering title filter (keyword search leaks sales). Newest-first; stop at first stale job. Walk until an empty page or a repeated first card. A hung listing (15s) uses cards already in the HTML, otherwise retries on a new tab; two failed listings stop that query. Skip detail when listing location/title is enough. Apply is on Ladders. |
...
| [RemoteWLB](https://remotewlb.com/jobs) | Job sitemaps | Guest monthly `/sitemaps/jobs/YYYY-MM-1` (listing HTML is a JS shell). Month urlsets are oldest-first — reverse to newest-first; stop at first stale `lastmod`. Engineering title filter. JobPosting JSON-LD for description/company. `location` is `Remote`. `directApply` false. |
