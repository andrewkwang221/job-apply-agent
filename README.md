...
| [Startup.jobs](https://startup.jobs/remote-jobs?w=remote&c=full-time%2Cpart-time%2Ccontractor&since=7d&page=1) | Playwright Chrome + JSON-LD | Guest remote FT/PT/contractor list. Cloudflare blocks listing HTML; installed Chrome is the default listing fetch. No `q=` (keyword search leaks non-eng titles). `since` from `max_job_age_days`. Engineering title filter. Newest-first; stop at first stale job. Skip detail when listing location/title is enough. JobPosting JSON-LD over HTTP (never `/apply/`). Apply is on Startup.jobs (capped at review). |
