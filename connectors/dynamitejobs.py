...
class DynamiteJobsConnector(BaseConnector):
    ...
    def fetch_jobs(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"dynamitejobs RSS XML failed ({type(e).__name__})")
            return []
...
def _fetch_feed() -> bytes | None:
    for attempt in range(1, _RETRIES + 1):
        try:
            ...
            logger.info(
                f"dynamitejobs RSS failed ({type(e).__name__}) "
        ...
        if response.status_code >= 400:
            logger.info(
                f"dynamitejobs RSS HTTP {response.status_code} "
                f"attempt {attempt}/{_RETRIES}"
            )
            if attempt < _RETRIES:
                time.sleep(_RETRY_DELAY * attempt)
    ...
    logger.info(f"dynamitejobs RSS skipped after {_RETRIES} attempts")
    return None
