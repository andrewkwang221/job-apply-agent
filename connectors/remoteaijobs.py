"""
Remote AI Jobs connector — Real Work From Anywhere AI category RSS.

Guest feed: GET https://www.realworkfromanywhere.com/remote-ai-jobs/rss.xml
(narrower than the main RWFA feed). Mixed dates — filter by
``max_job_age_days``, no first-stale stop. Engineering title filter.
...
def _fetch_feed() -> bytes | None:
    for attempt in range(1, _RETRIES + 1):
        try:
            ...
            logger.info(
                f"remoteaijobs RSS failed ({type(e).__name__}) "
        ...
        if response.status_code >= 400:
            logger.info(
                f"remoteaijobs RSS HTTP {response.status_code} "
                f"attempt {attempt}/{_RETRIES}"
            )
            if attempt < _RETRIES:
                time.sleep(_RETRY_DELAY * attempt)
    ...
    logger.info(f"remoteaijobs RSS skipped after {_RETRIES} attempts")
    return None
